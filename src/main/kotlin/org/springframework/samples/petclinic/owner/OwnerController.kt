...
class OwnerController(private val owners: OwnerRepository) {
    ...
    fun findOwner(@PathVariable(name = "ownerId", required = false) ownerId: Int?): Owner {
        return if (ownerId == null) {
            ...
            this.owners.findById(ownerId)
                .orElseThrow { 
                    IllegalArgumentException("Owner not found with id: $ownerId. Please ensure the ID is correct and the owner exists in the database.")
                }
        }
    }
    ...
    fun showOwner(@PathVariable("ownerId") ownerId: Int): ModelAndView {
        ...
        val optionalOwner = this.owners.findById(ownerId)
